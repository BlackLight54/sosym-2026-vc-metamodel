...
\begin{table}
\caption{Scalability measurements across three Refinery operations.
\emph{Consistency} (\texttt{check}): verifies partial model has no
internal contradictions. \emph{Concretizability} (\texttt{check\ -k}):
determines whether a concrete model satisfying all constraints exists.
\emph{Generation} (\texttt{generate}): produces a concrete model
instance. Wall-clock seconds, mean \(\pm\sigma\) over 10 runs.
