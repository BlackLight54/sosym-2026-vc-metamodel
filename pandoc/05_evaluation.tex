...
\tightlist
\item
  \textbf{eIDAS \ac{ARF}} (normative, SHALL): EU wallet attestations must be issued in SD-JWT-VC or mdoc format, i.e.~\(\text{format}(\text{IncomeCred}) \in \{\text{SD-JWT-VC}, \text{mdoc}\}\) \citep{noauthor_eu-digital-identity-walleteudi-doc-architecture-and-reference-framework_2026}. Neither format supports predicate proofs; both provide selective disclosure only.
\item
...
\paragraph{Soundness: if the framework reports a constraint violation, the (partial) model necessarily violates it; no false positives arise from open design decisions}\label{soundness-if-the-framework-reports-a-constraint-violation-the-partial-model-necessarily-violates-it-no-false-positives-arise-from-open-design-decisions}

\paragraph{\texorpdfstring{Completeness with respect to the formalized predicates: every anti-pattern in \autoref{tab:antipatterns} is checked exhaustively over all model elements, so a flaw matching a defined predicate cannot escape detection. These guarantees distinguish the approach from random graph generation (no constraint satisfaction guarantees), LLM-based schema generation (no formal guarantees), and manual inspection (cannot systematically cover cross-layer interactions at scale)}{Completeness with respect to the formalized predicates: every anti-pattern in  is checked exhaustively over all model elements, so a flaw matching a defined predicate cannot escape detection. These guarantees distinguish the approach from random graph generation (no constraint satisfaction guarantees), LLM-based schema generation (no formal guarantees), and manual inspection (cannot systematically cover cross-layer interactions at scale)}}\label{completeness-with-respect-to-the-formalized-predicates-every-anti-pattern-in-is-checked-exhaustively-over-all-model-elements-so-a-flaw-matching-a-defined-predicate-cannot-escape-detection.-these-guarantees-distinguish-the-approach-from-random-graph-generation-no-constraint-satisfaction-guarantees-llm-based-schema-generation-no-formal-guarantees-and-manual-inspection-cannot-systematically-cover-cross-layer-interactions-at-scale}
