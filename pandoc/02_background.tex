...
::: todo
cite this:
\citet{incollection}\{Belnap77useful,
...
:::
The four-valued interpretation assigns each element one of four statuses: \emph{true} (the value must be true), \emph{false} (the value must be false), \emph{unknown} (possibly true or false), and \emph{error} (both true and false, denoting contraiction). In diagrams,
::: todo
do we have diagrams?
:::
a solid line denotes a true values, a dashed line denotes unknown values, and absence denotes a false value. During the development (or automated synthesis) of partial models \emph{unknown} values are gradually refined to either \emph{true} or \emph{false}.
