...
The gap analysis (2026-03-25) found that MDE **has** been applied to the SSI domain — four prior works exist (ChorSSI, SecureSSI, Pattiyanon et al., Barclay et al.). DECISIONS.md records the corrected claim (2026-03-25, "Claim C5 correction"):
...
1. `DECISIONS.md` — 2026-03-25 entry "Claim C5 correction: MDE has been applied to SSI" (exact corrected wording)
...
4. `archive/gap analyis/GAP_ANALYSIS_SYNTHESIS.md` — the four MDE-for-SSI papers and their differentiation
...
2. Replace with the corrected claim from DECISIONS.md. The replacement should:
...
2. The current wording is: "no existing tool checks their cross-layer consistency." This is scoped to tools/checking, which may already be defensible. Evaluate:
   - If the sentence claims "no MDE/metamodel for VC" → rewrite to match DECISIONS.md.
...
- The corrected claim must match the DECISIONS.md version in substance, even if phrased differently for flow.
