...
\todo[inline,color=orange!20]{figure fig\_teaser: Teaser figure (full-width, before first column). Split panel: Left --- Diploma scenario showing vertical constraint (format{\textrightarrow}claim ZKP dependency). Right --- Alice scenario showing horizontal constraint (cross-credential entity alignment). Both highlight cross-layer errors invisible to single-layer inspection.}
