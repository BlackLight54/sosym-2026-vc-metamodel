...
\todo[color=orange!20]{figure fig_teaser: Teaser figure (full-width, before first column). Split panel: Left — Diploma scenario showing vertical constraint (format→claim ZKP dependency). Right — Alice scenario showing horizontal constraint (cross-credential entity alignment). Both highlight cross-layer errors invisible to single-layer inspection.}

\todo[color=blue!20]{cite: W3C VCDM 2.0}

\todo[color=blue!20]{cite: eIDAS 2.0 / EU Digital Identity Wallet}

\todo[color=blue!20]{cite: Hyperledger AnonCreds / Aries}
...
\todo[color=blue!20]{cite: existing VC design tools/approaches that are single-layer}
...
\todo[color=gray!20]{Structure paragraph — regenerate with skills/structure_paragraph when sections change.}
