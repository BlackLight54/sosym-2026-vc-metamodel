...
0 inexpressible) is copied from the vault claim `CLM-expressiveness_validation` and the
version-pinned source note `SRC-eudiw_arf_n2_n9_n0`; no constraint was re-classified here, and no
requirement text was re-fetched. The file exists because §5.1.2 asserts a per-constraint mapping
that no artifact previously exhibited (DEC-overpromise_until_delivered register: the promise register stays, the delivery
work is done).

**Source pin.** eIDAS ARF **v2.9.0**, released 21 May 2026, re-verified against the primary text on
2026-07-13 (SRC-eudiw_arf_n2_n9_n0). Requirement identifiers cite ARF Annex 2.02 (high-level requirements by topic);
section numbers cite the ARF main document at v2.9.0. Per DEC-dated_snapshot_method_over_catalogue (dated snapshot, method over
catalogue) this pin is deliberate and dated, not a currency claim. **Currency flag:** ARF v3.0.0 was
released 2026-07-22 and is not covered by this table (SRC-eudiw_arf_n2_n9_n0 currency alert; open item for the
...
| ID | ARF v2.9.0 locus | Obligation (paraphrase; verbatim texts in SRC-eudiw_arf_n2_n9_n0 / CPT-dual_issuance_mandate) | Encoding in `models/` | Verdict (CLM-expressiveness_validation) |
|---|---|---|---|---|
| ARF-C1 | PID_02, Annex 2.02 Topic 3 (A.2.3.2) | A PID Provider SHALL issue any PID in **both** ISO/IEC 18013-5 (mdoc) **and** SD-JWT VC. Conjunctive, not disjunctive (CPT-dual_issuance_mandate). | `EidasMandate` (`vc_metamodel.refinery:304`) marks the credential; the two mandated families exist as `MdocSchema` and `SdJwtVcSchema` (`:202`, `:201`); assignment runs through `Credential.format` (`:179`). **Fidelity gap, see note 1:** `contains Formatted_Credential[1] format` is arity 1, so one credential cannot carry two format assignments at once. | Full |
| ARF-C2 | ARB_01, Annex 2.02 Topic 12 (A.2.3.9) | QEAA / PuB-EAA: two formats admitted (mdoc, SD-JWT VC); W3C VCDM is not admitted. | **Not encoded.** Eligibility is conditioned on attestation qualification level, which the metamodel does not distinguish (root cause GAPQ-attestation_qualification_levels). Nearest elements: the `Formatted_Credential` subclasses (`:198`–`:202`), which carry no qualification tier. | Partial |
| ARF-C3 | ARB_01a, Annex 2.02 Topic 12 (A.2.3.9) | Non-qualified EAA: three formats admitted; W3C VCDM added relative to ARB_01. | **Not encoded.** Same root cause as C2 (GAPQ-attestation_qualification_levels): the model cannot state "non-qualified" and therefore cannot state the widened admissibility. | Partial |
...
| ARF-C5 | PID_21, Annex 2.02 Topic 3 (A.2.3.2) | Per-claim selective disclosability: the User must be able to disclose attributes individually. | **Not encoded at the required granularity.** `supports_selective_disclosure` (`:212`) is a *format*-level capability and `PrivacyRequirement` (`:309`) targets a *whole credential*; neither expresses a per-claim disclosure obligation (root cause GAPQ-claim_granularity_privacy_annotations). | Partial |
| ARF-C6 | Main doc §5.4.4 (was §5.3.4 in v2.7.3) | W3C VCDM conformance as a meta-level property of the format profile, admitted only for the non-qualified tier. | **Not encoded as a meta-level property.** `conforms_vcdm` (`:220`) is a boolean over format classes and `VcdmConformance` (`:314`) is a per-credential requirement marker; the tier-conditioned meta-level reading needs the qualification level (root cause GAPQ-attestation_qualification_levels). The enforced reading of the requirement side exists only as the standalone instrument `spec_ambiguity.refinery` (`vcdm_conformance_violation`), which no CSOK entry point imports (DEC-c7_available_instrument_wording). | Partial |
...
| ARF-C8 | Main doc §5.4.3 (salted-hash mechanism, was §5.3.3); §7.4.3.5.3 (ZKP contrast, number stable) | Salted-hash selective disclosure leaves Attestation Provider linkability irreducible; ZKPs are the only viable mitigation. | **Not encoded.** The distinction is between disclosure *mechanisms* at claim granularity; the model carries `supports_selective_disclosure` (`:212`) and `supports_zkp` (`:226`) as format-level booleans and cannot express which claim is disclosed by which mechanism (root cause GAPQ-claim_granularity_privacy_annotations). | Partial |

**Totals: 3 encoded (C1, C4, C7), 5 not encoded (C2, C3, C5, C6, C8), 0 inexpressible.** Identical
to the CLM-expressiveness_validation classification. "Not encoded" is a statement about the delivered model, not about
expressive reach: CLM-expressiveness_validation's claim is that each partial closes under one metaclass extension
(GAPQ-attestation_qualification_levels attestation qualification levels; GAPQ-claim_granularity_privacy_annotations claim-granularity privacy annotations) without
...
|---|---|---|
| Attestation qualification level (PID / QEAA / non-qualified EAA) undistinguished | C2, C3, C6 | GAPQ-attestation_qualification_levels — qualification metaclasses on the FSL side, so format eligibility can be conditioned on tier |
| Privacy annotation lives at format and credential level, not claim level | C5, C8 | GAPQ-claim_granularity_privacy_annotations — a claim-level privacy annotation, so per-claim disclosability and mechanism are statable |

## Notes

**Note 1 — ARF-C1 and format arity.** The dual-issuance obligation is conjunctive (CPT-dual_issuance_mandate: "in both
...
supplement records the mismatch and does not resolve it**; the C1 verdict column reproduces CLM-expressiveness_validation
...
- `zettelkasten/sources/SRC-eudiw_arf_n2_n9_n0.md` — version pin, ARF-C1..C8 re-verification table,
  verbatim requirement texts, section renumbering, v3.0.0 currency alert.
- `zettelkasten/claims/CLM-expressiveness_validation.md` — the classification this table
  reproduces.
- `zettelkasten/concepts/CPT-dual_issuance_mandate.md`, `CPT-eidas_arf_format_mandate.md` —
  the conjunctive PID mandate and the format-mandate concept.
- `zettelkasten/questions/GAPQ-attestation_qualification_levels.md`,
  `GAPQ-claim_granularity_privacy_annotations.md` — the two root causes.
- `zettelkasten/decisions/DEC-dated_snapshot_method_over_catalogue.md` — why the pin is dated.
