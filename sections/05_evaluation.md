...
The [governance_conflict]{.refi} error predicate (\autoref{lst:governance-conflict}) encodes the conflict directly. Its two disjunctive clauses capture the core tension: clause 1 fires for +VCDM-conformant formats lacking predicate proofs (SD-JWT-VC, JSON-LD, JWT-VC); clause 2 fires for predicate-proof-capable formats outside +VCDM (AnonCreds). The remaining format (mdoc) lacks both capabilities and is eliminated by the privacy requirement. No format escapes all constraints, so the configuration is unsatisfiable. The constraint sensitivity analysis (\autoref{sec:scalability}) confirms this: of eight governance subsets, only the full conjunction yields unsatisfiability; every proper subset admits at least one valid format assignment. The only available workaround, pre-computing boolean threshold claims in SD-JWT-VC, requires changing the domain concept layer's information model (\autoref{sec:cross-layer}), a cross-layer design consequence visible only under multi-layer analysis.
