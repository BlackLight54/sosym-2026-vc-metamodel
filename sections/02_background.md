...
[^other-formats]: JSON-LD with BBS [@noauthor_verifiable_2024; @_bbs_2024] and ISO mdoc [@_mobile_2021] also support selective disclosure but not predicate proofs.
...
<!-- CUT-1: commented out redundant truth-value re-explanation (values already defined in Partial Model definition above)
...
By convention, diagrams render [true]{.refi} values as solid lines, [unknown]{.refi} as dashed lines, and [false]{.refi} as absence. During refinement, [unknown]{.refi} values are committed to [true]{.refi} or [false]{.refi}; a model with no remaining [unknown]{.refi} values is *concrete*. An [error]{.refi} value arises when a design decision contradicts a constraint.

::: {.formal}
A partial model $A$ *refines* $B$ if every [true]{.refi} or [false]{.refi} assignment in $B$ is preserved in $A$; [unknown]{.refi} values in $B$ may be committed to either [true]{.refi} or [false]{.refi} in $A$. Refinement is a partial order over interpretations that formalizes the monotonic narrowing of design alternatives [@semerath_graph_2018].
:::
...
::: {.formal}
Formally, a propagation rule fires when its precondition pattern matches with all bindings committed to [true]{.refi}; the consequent then forces the target element to [true]{.refi} (positive inference) or [false]{.refi} (negative elimination). Rules are applied to a fixpoint: the engine iterates until no rule produces a new assignment, yielding a refined partial model that preserves all prior commitments [@semerath_graph_2018; @marussy_refinery_2024].
:::
...
::: {.formal}
Semeráth et al. establish these properties formally for partial graph specifications: soundness guarantees that every generated instance satisfies all metamodel constraints and graph predicates, while bounded completeness ensures that if no valid model exists within the declared scope, the solver reports unsatisfiability [@semerath_graph_2018]. The multi-layer metamodel and cross-layer predicates defined in \autoref{sec:approach} are expressed as standard Refinery specifications, so generated credential ecosystem models inherit both guarantees.
:::
