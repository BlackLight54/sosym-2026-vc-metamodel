...
Table: DCL instance for the housing subsidy scenario.
...
The first two predicates mirror DCL's `statement` and root-entity classification at the credential level. The two error predicates catch structural violations that would otherwise propagate silently to downstream format assignment. When either fires, the framework reports the violation (e.g., $\text{NOT\_OK}(\text{no\_empty\_cred}(cs))$) before format-specific constraints are even evaluated. The full constraint definitions are provided in the supplementary material. In terms of the usage modes (\autoref{sec:functional-overview}), the shadow predicates (`credential_statement`, `Root_cred_entity`) derive structural facts in all three modes; the error predicates (`no_empty_cred`, `root_ent_doesnt_have_cred`) trigger in consistency checking and error identification.

In the housing subsidy scenario, three credentials partition the claim-layer facts:

| Credential | CredentialSubject | Claim | CredentialValue |
|---|---|---|---|
| FamilyStatusCred | $\text{CS\_Applicant}_1$ | $\text{has\_children}_1$ | $\text{num\_children}_1$ |
| PropertyCred | $\text{CS\_Applicant}_2$ | $\text{owns\_property}_1$ | $\text{property\_area}_1$ |
| IncomeCred | $\text{CS\_Applicant}_3$ | $\text{earns}_1$ | $\text{monthly\_income}_1$ |

Table: CSL instance for the housing subsidy scenario. Issuers: civil registry (FamilyStatusCred), land registry (PropertyCred), employer (IncomeCred).
...
Table: FSL assignments for the housing subsidy scenario.
...
Format capability propagation rules complete the cross-layer mechanism. When a governance annotation or domain requirement demands a capability, propagation rules eliminate incompatible format classes by negative elimination: for a `Formatted_Credential` $f$ where `supports_predicate_proof` must hold, any format class that cannot provide predicate proofs (SD-JWT-VC, JWT-VC, mdoc, JSON-LD) is set to **false**, leaving only AnonCreds. The mechanism is the same as the trace consistency rules (`prop_t`, `prop_s`): Refinery's four-valued interpretation distinguishes **must** (committed true), **must not** (committed false), **may** (possibly true), and **may not** (possibly false), and propagation rules convert open assignments to committed false when a constraint excludes them. The key distinction is between propagation rules, which guide generation by narrowing the design space, and error predicates such as `governance_conflict`, which check committed assignments and report violations. This distinction enables the metamodel to serve both as a consistency checker for existing designs and as an efficient generative tool for design space exploration.

The contribution of this section is the cross-layer constraint methodology, not the individual layer definitions. Single-layer schemas for credentials and format capability matrices exist in the literature. The distinguishing element is expressing constraints that span metamodel layers as graph predicates evaluable by a partial model solver, detecting conflicts that no single layer reveals when checked independently.
