...
Dependencies: Section 02 (Refinery concepts, multi-level modeling), Section 03 (running example).
...
<!-- Job: Instantiate the housing subsidy example at this layer. -->
<!-- Key content: Applicant (Subject) with three Props. Connected graph. Cross-property domain constraint. Satisfies all claim-layer constraints. -->
...
<!-- Job: Instantiate the housing subsidy example at this layer. Show three credentials with trace mappings and entity alignment. -->
...
<!-- Note: This is where the housing subsidy example is primary — format choice constrains privacy capabilities. -->
...
<!-- Job: Show format assignments for the housing subsidy credentials and the governance conflict site. -->
...
<!-- Job: Classify constraints by source and scope, instantiated with CSOK. -->
...
<!-- Job: Apply the complete constraint set to the housing subsidy example. Show what the model generator produces. -->
<!-- Key content: Housing subsidy example — apply all constraints, show which propagation rules fire, show the generated model. Show the cross-layer constraint violation when SD-JWT-VC is chosen but predicate proof is required. -->
...
<!-- A17: Format-driven CPL restructuring — worked example of cross-layer propagation -->
<!-- Job: Show concretely how FSL limitations propagate upward to CPL, changing the claim structure. -->
<!-- Key content: AnonCreds encodes monthly_income as integer, predicate proof works directly. SD-JWT-VC forces boolean restructuring (income_above_X claims). This is a cross-layer constraint propagation. -->
<!-- Placement: Sec 04.4 worked example (per DECISIONS.md). -->
