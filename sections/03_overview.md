...
\begin{figure*}[t]
  ...
  \caption{The proposed multi-layer modeling framework applied to a housing subsidy credential ecosystem across three metamodel layers, from domain facts through credential schemas to format-specific representations. }
  \Description{Three-layer instance diagram for the housing subsidy (CSOK) scenario. The domain concept layer shows three entity groups (family status, property, income) with a value dependency linking floor area to number of children. The credential schema layer distributes these across three credentials (FamilyStatusCred, PropertyCred, IncomeCred) issued by independent authorities, with trace links to domain properties. The format-specific layer shows format assignments; the income credential is highlighted with conflicting governance annotations (eIDAS mandate requiring SD-JWT-VC versus privacy requirement demanding predicate proof). A horizontal cross-credential edge marks the floor area constraint spanning two credentials.}
  \label{fig:teaser}
\end{figure*}
```
