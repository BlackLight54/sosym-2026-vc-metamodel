...
Do NOT insert the markers — present them for Martin's approval. If Martin says "do it," use `skills/plan_revision_orchestration` to generate ordered edit prompts.
...
- Reference prompt files if `skills/plan_revision_orchestration` is used for execution.
...
- **`skills/review_paper_status`:** Paper status reports section maturity; budget_cut reports section *size*. Complementary — run paper_status first for context.
- **`skills/plan_revision_orchestration`:** If cuts touch multiple sections, use revision_orchestration to decompose the edits into ordered prompts.
- **`skills/review_claim_evidence_audit`:** Before dropping a contribution (Phase 3), run claim_evidence_audit to understand what evidence chain breaks.
- **`skills/draft_figure_design`:** If figure creep is diagnosed, coordinate with figure_design to consolidate or resize figures.
...
- **Before submission** — final budget check alongside `skills/review_pre_submission_check`.
