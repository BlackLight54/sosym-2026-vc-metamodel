...
|-------------|-------------|--------|
| **Stale** | The paper has moved past this decision but it hasn't been formally updated | Update the decision, propagate via `skills/revision_orchestration` |
...
Martin decides what to change. Changes are propagated via `skills/revision_orchestration` if they touch multiple sections.
...
- **`skills/revision_orchestration`:** If a decision changes, the revision orchestrator propagates it across sections.
- **`skills/claim_evidence_audit`:** Decision drift often manifests as claim-evidence mismatches. If the audit finds problems, a decision audit may explain why.
- **`skills/champion_test`:** If the champion insight has shifted, the contribution framing decision (D3-type) may need updating.
- **`skills/paper_status`:** Paper status is a snapshot of *where things are*. Decision audit is a check on *whether the plan is still right*.
