...
- `skills/evaluation_design` output (if it was run).
- Contribution list from Introduction P3.
- CLAUDE.md: thesis and domain.
- DECISIONS.md: any evaluation-related decisions.
...
| E3 | Supply chain case study | Case study | Not started | Running example (DECISIONS.md) | All steps traced | §5.2 |
...
- **`skills/evaluation_design`:** Designs the evaluation (what to evaluate, which methods). This skill tracks execution of that plan. Run evaluation_design first if no evaluation plan exists.
- **`skills/claim_evidence_audit`:** Audits whether claims have proportional evidence. If the audit finds gaps, they may map to incomplete items on this checklist.
- **`skills/limitations_threats`:** Limitations often arise from evaluation scope decisions. Items marked as out-of-scope on this checklist may become limitations.
- **`skills/paper_status`:** Paper status reports section maturity. This skill reports evaluation completeness — a more granular view of the evaluation section's readiness.
