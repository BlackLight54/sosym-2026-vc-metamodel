...
- The change to be made (from Martin, from `skills/review_final_review`, from `skills/review_claim_evidence_audit`, or from reviewer feedback).
...
- [ ] Run `skills/review_pre_submission_check` to catch broken refs.
...
When processing multiple reviewer points (from `skills/plan_rebuttal`), each point may generate its own revision plan. If two revision plans touch the same section, merge them into a single plan with the combined edits — don't create conflicting prompts for the same file.
