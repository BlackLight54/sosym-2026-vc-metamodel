...
This is not a quality review (that's `skills/review_final_review`). This is a dashboard: fast, factual, actionable.
...
- `context/todos/INDEX.md` — todo overview.
...
Read `context/todos/INDEX.md`. Check for drift:
...
Read MEMORY.md for decision and claim summaries:
- **Active decisions:** Count and list.
- **Binding claims:** For each `claim_*.md` entry, report delivery status (not_started / partial / delivered / weak).
- **Late-start detection:** If >50% of claims are undelivered with <14 days remaining, flag late-start mode.
...
| Running example chosen | Yes (check decision memories) / No |
...
If decision/claim memories or completed todos contain notes from previous sessions, compare:
- Which sections advanced since last session?
- Which markers were resolved?
- Which binding claims changed status?
...
- **`skills/review_final_review`:** Paper status is a lightweight check; final review is a deep read. Don't confuse them.
- **`skills/review_pre_submission_check`:** Paper status counts markers; pre_submission_check diagnoses them.
